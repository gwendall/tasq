...
function assertKnownFlags(command: string, args: ReturnType<typeof parseArgs>): void {
  const byCommand: Record<string, readonly string[]> = {
    ...
    done: ["reason", "note", "source", "at", "evidence", "decision", "expected-revision", "idempotency-key"],
    complete: ["reason", "note", "source", "at", "evidence", "decision", "expected-revision", "idempotency-key"],
