...
describe("migration data-safety envelope", () => {
  it("creates a verified private snapshot and complete durable receipt", async () => {
    ...
    try {
      ...
      expect(result).toMatchObject({
        ...
        irreversible: true,
        receipt: { status: "complete", sourceFormat: 5, targetFormat: 31 },
      });
      ...
      expect(receipt).toMatchObject({
        ...
        postVerification: { schemaFormat: 31, service: { ok: true, issues: [] } },
      });
  ...
  it("serializes concurrent first-open upgrades and preserves both receipts", async () => {
    ...
    try {
      ...
      expect(results.reduce((count, result) => count + result.applied.length, 0)).toBe(26);
      ...
      expect((await left.client.execute("SELECT count(*) AS count FROM _migration")).rows[0]?.count).toBe(32);
    } finally {
      await left.close();
      await right.close();
    }
  ...
  it("recovers real process kills at every migration safety boundary", async () => {
    ...
    for (const boundary of boundaries) {
      ...
      try {
        ...
        expect(result.afterFormat, boundary).toBe(31);
        expect((await resumed.client.execute("SELECT count(*) AS count FROM _migration")).rows[0]?.count).toBe(32);
  ...
  it("fails closed under a real file-size quota before schema mutation", async () => {
    ...
    try {
      ...
      expect(resumed).toMatchObject({ afterFormat: 31, receipt: { status: "complete" } });
    } finally {
  ...
  it("finalizes a committed receipt left pending by a process stop", async () => {
    ...
    try {
      ...
      expect(resumed.receipt).toMatchObject({ status: "complete", sourceFormat: 5, targetFormat: 31 });
      expect(JSON.parse(readFileSync(first.receipt!.path, "utf8"))).toMatchObject({
        status: "complete",
        recoveredAfterRestart: true,
        postVerification: { schemaFormat: 31, service: { ok: true } },
      });
    } finally {
      await opened.close();
    }
  ...
  it("retains the recovery point and returns a restore plan when post-check fails", async () => {
    ...
    try {
      ...
      expect(problem!.toJSON()).toMatchObject({
        ...
        receipt: { status: "failed", sourceFormat: 5, targetFormat: 31 },
        restore: {
