...
async function buildCandidate(directory: string): Promise<ReturnType<typeof releasePaths>> {
  const version = publishedReleaseDirectory === undefined ? "0.1.0" : publishedReleaseVersion;
  if (publishedReleaseDirectory !== undefined) {
    const paths = releasePaths(publishedReleaseDirectory, version);
...
describe.skipIf(target === null)("TQ-606 blind public adoption", () => {
  ...
  expect((await ok(cli, ["--version"], { cwd: publicEntry, home })).trim())
