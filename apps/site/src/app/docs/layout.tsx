...
export default function DocsLayout({ children }: { children: React.ReactNode }) {
  return (
    <main id="main-content" className="site-container grid min-h-[calc(100vh-4rem)] lg:grid-cols-[240px_1fr]">
      <aside className="border-b border-[var(--line)] py-7 lg:border-r lg:border-b-0 lg:pr-7">
        <nav aria-label="Documentation" className="sticky top-24 grid grid-cols-2 gap-x-4 gap-y-7 sm:grid-cols-3 lg:grid-cols-1">
          {groups.map((group) => (
            <div key={group.label}>
              <p className="mb-2 font-mono text-[0.62rem] font-semibold uppercase tracking-[0.12em] text-[var(--ink-faint)]">{group.label}</p>
