#!/bin/sh
# Tasq Local v0.3.0 verified lifecycle bootstrap.
#
# Inspect first:
...
usage() {
  cat <<'EOF'
Usage: install.sh [--version 0.3.0] [--prefix PATH] [--dry-run]
                  [--activate|--uninstall]
...
[ "$VERSION" = "0.3.0" ] || fail "this versioned bootstrap supports only Tasq 0.3.0"
[ -n "$PREFIX" ] || fail "--prefix must not be empty"

case "$(uname -s):$(uname -m)" in
  Darwin:arm64)
    TARGET="darwin-arm64"
    CHECKSUMS_SHA256="4f78f71e1a258ccf8c154124e4b8372e24102601c22cb0e3927ebfcf322ffb9a"
    ;;
  Linux:x86_64)
    TARGET="linux-x64-gnu"
    CHECKSUMS_SHA256="10fa567c06ff87dd14cb456fec7d0ff4d0b9e714e51bab74e16bb869db8d3992"
    ;;
  *)
