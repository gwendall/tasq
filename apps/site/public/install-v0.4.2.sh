#!/bin/sh
# Tasq Local v0.4.2 verified lifecycle bootstrap.
#
# Inspect first:
...
usage() {
  cat <<'EOF'
Usage: install.sh [--version 0.4.2] [--prefix PATH] [--dry-run]
                  [--activate|--uninstall]
...
[ "$VERSION" = "0.4.2" ] || fail "this versioned bootstrap supports only Tasq 0.4.2"
[ -n "$PREFIX" ] || fail "--prefix must not be empty"

case "$(uname -s):$(uname -m)" in
  Darwin:arm64)
    TARGET="darwin-arm64"
    CHECKSUMS_SHA256="66156d404a9b843f98b69f2bcc22522800f7fc737977bf6e6acb94ccf595e9ce"
    ;;
  Linux:x86_64)
    TARGET="linux-x64-gnu"
    CHECKSUMS_SHA256="13623379145889cd773c2cb6b2f648d0e28ba053a30bb12135c97b4fdc9dcb6f"
    ;;
  *)
