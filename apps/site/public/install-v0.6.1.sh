#!/bin/sh
# Tasq Local v0.6.1 verified lifecycle bootstrap.
#
# Inspect first:
...
#
# Install:
#   sh /tmp/tasq-install.sh --version 0.6.1 --prefix "$HOME/.local"
...
usage() {
  cat <<'EOF'
Usage: install.sh [--version 0.6.1] [--prefix PATH] [--dry-run]
                  [--activate|--uninstall]
...
[ "$VERSION" = "0.6.1" ] || fail "this versioned bootstrap supports only Tasq 0.6.1"
[ -n "$PREFIX" ] || fail "--prefix must not be empty"

case "$(uname -s):$(uname -m)" in
  Darwin:arm64)
    TARGET="darwin-arm64"
    CHECKSUMS_SHA256="6c48269c5835ec47d77012820d1b40c42873c581fecb18b5f1bd1cb0f7dabb1e"
    ;;
  Linux:x86_64)
    TARGET="linux-x64-gnu"
    CHECKSUMS_SHA256="d2b22824d31c3544a9e3a242c0c1d4d9731658f07f72cdd23ca788b3dcdb90e0"
    ;;
  *)
