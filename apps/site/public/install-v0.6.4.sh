#!/bin/sh
# Tasq Local v0.6.4 verified lifecycle bootstrap.
#
# Inspect first:
...
#
# Install:
#   sh /tmp/tasq-install.sh --version 0.6.4 --prefix "$HOME/.local"
...
usage() {
  cat <<'EOF'
Usage: install.sh [--version 0.6.4] [--prefix PATH] [--dry-run]
                  [--activate|--uninstall]
...
[ "$VERSION" = "0.6.4" ] || fail "this versioned bootstrap supports only Tasq 0.6.4"
[ -n "$PREFIX" ] || fail "--prefix must not be empty"

case "$(uname -s):$(uname -m)" in
  Darwin:arm64)
    TARGET="darwin-arm64"
    CHECKSUMS_SHA256="82433614d8d9e6955ca0b6b28972091a0c05c00833325a389b76511cad0a405c"
    ;;
  Linux:x86_64)
    TARGET="linux-x64-gnu"
    CHECKSUMS_SHA256="ca6ffe734c8cc562adedc7a73da149efa85b41cb54a56990c0fc52dea69ded75"
    ;;
  *)
