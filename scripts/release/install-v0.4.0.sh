#!/bin/sh
# Tasq Local v0.4.0 verified lifecycle bootstrap.
#
# Inspect first:
...
usage() {
  cat <<'EOF'
Usage: install.sh [--version 0.4.0] [--prefix PATH] [--dry-run]
                  [--activate|--uninstall]
...
[ "$VERSION" = "0.4.0" ] || fail "this versioned bootstrap supports only Tasq 0.4.0"
[ -n "$PREFIX" ] || fail "--prefix must not be empty"

case "$(uname -s):$(uname -m)" in
  Darwin:arm64)
    TARGET="darwin-arm64"
    CHECKSUMS_SHA256="6aea7d565a52eccceaeb3702c405868b7c047e052b6c9d7b5c90e804dbcc74b0"
    ;;
  Linux:x86_64)
    TARGET="linux-x64-gnu"
    CHECKSUMS_SHA256="b3d873944a7baa7764f18523bfd359be26805d3d7e805b90c53e08255d2729bf"
    ;;
  *)
