#!/bin/sh
# Tasq Local v0.5.1 verified lifecycle bootstrap.
#
# Inspect first:
...
#
# Install:
#   sh /tmp/tasq-install.sh --version 0.5.1 --prefix "$HOME/.local"
...
usage() {
  cat <<'EOF'
Usage: install.sh [--version 0.5.1] [--prefix PATH] [--dry-run]
                  [--activate|--uninstall]
...
[ "$VERSION" = "0.5.1" ] || fail "this versioned bootstrap supports only Tasq 0.5.1"
