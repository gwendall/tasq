#!/bin/sh
# Tasq Local v0.6.8 verified lifecycle bootstrap.
#
# Inspect first:
...
#
# Install:
#   sh /tmp/tasq-install.sh --version 0.6.8 --prefix "$HOME/.local"
...
usage() {
  cat <<'EOF'
Usage: install.sh [--version 0.6.8] [--prefix PATH] [--dry-run]
                  [--activate|--uninstall]
...
[ "$VERSION" = "0.6.8" ] || fail "this versioned bootstrap supports only Tasq 0.6.8"
[ -n "$PREFIX" ] || fail "--prefix must not be empty"

case "$(uname -s):$(uname -m)" in
  Darwin:arm64)
    TARGET="darwin-arm64"
    CHECKSUMS_SHA256="aa7677cbcf0f6fb75a129e80fe4b503d889c955e6edddaf42df94517c655abf1"
    ;;
  Linux:x86_64)
    TARGET="linux-x64-gnu"
    CHECKSUMS_SHA256="02616a329e180c1919b6abd6e8682c8348731274f691d6d0b3cb045aaec5bbe2"
    ;;
  *)
