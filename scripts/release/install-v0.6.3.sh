#!/bin/sh
# Tasq Local v0.6.3 verified lifecycle bootstrap.
#
# Inspect first:
...
#
# Install:
#   sh /tmp/tasq-install.sh --version 0.6.3 --prefix "$HOME/.local"
...
usage() {
  cat <<'EOF'
Usage: install.sh [--version 0.6.3] [--prefix PATH] [--dry-run]
                  [--activate|--uninstall]
...
[ "$VERSION" = "0.6.3" ] || fail "this versioned bootstrap supports only Tasq 0.6.3"
[ -n "$PREFIX" ] || fail "--prefix must not be empty"

case "$(uname -s):$(uname -m)" in
  Darwin:arm64)
    TARGET="darwin-arm64"
    CHECKSUMS_SHA256="e0555ace4330a60d37e918f76db92ee6e79f415ad103e863c8ff3bfe0918bdf9"
    ;;
  Linux:x86_64)
    TARGET="linux-x64-gnu"
    CHECKSUMS_SHA256="e97b22a28eb09eb98d6afa3f131b69b6a0b338d36c366cd32ebfac3a9d2283b0"
    ;;
  *)
