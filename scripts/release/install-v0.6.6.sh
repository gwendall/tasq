#!/bin/sh
# Tasq Local v0.6.6 verified lifecycle bootstrap.
#
# Inspect first:
...
usage() {
  cat <<'EOF'
Usage: install.sh [--version 0.6.6] [--prefix PATH] [--dry-run]
                  [--activate|--uninstall]
...
[ "$VERSION" = "0.6.6" ] || fail "this versioned bootstrap supports only Tasq 0.6.6"
[ -n "$PREFIX" ] || fail "--prefix must not be empty"

case "$(uname -s):$(uname -m)" in
  Darwin:arm64)
    TARGET="darwin-arm64"
    CHECKSUMS_SHA256="889fb39ffefe34ca1a1424544361a763e53f37e4ad1cf8c8ddcbc65566d1d65f"
    ;;
  Linux:x86_64)
    TARGET="linux-x64-gnu"
    CHECKSUMS_SHA256="1ae49130513d410214f28265b345da9deb358ca07fd9d82ed16e10038c7ef1e5"
    ;;
  *)
