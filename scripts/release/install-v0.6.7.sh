#!/bin/sh
# Tasq Local v0.6.7 verified lifecycle bootstrap.
#
# Inspect first:
...
#
# Install:
#   sh /tmp/tasq-install.sh --version 0.6.7 --prefix "$HOME/.local"
...
usage() {
  cat <<'EOF'
Usage: install.sh [--version 0.6.7] [--prefix PATH] [--dry-run]
                  [--activate|--uninstall]
...
[ "$VERSION" = "0.6.7" ] || fail "this versioned bootstrap supports only Tasq 0.6.7"
[ -n "$PREFIX" ] || fail "--prefix must not be empty"

case "$(uname -s):$(uname -m)" in
  Darwin:arm64)
    TARGET="darwin-arm64"
    CHECKSUMS_SHA256="e1e12f0a8833a5a63ad964aec0a80abbafb0f510a247c07e306d8c24b0df68ac"
    ;;
  Linux:x86_64)
    TARGET="linux-x64-gnu"
    CHECKSUMS_SHA256="db109526012ef519940cbeb3aa64d9ccff013f409896f5d9941b66157aefa8f1"
    ;;
  *)
