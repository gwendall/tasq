...
```bash
npx @tasq-run/cli@0.6.8 demo    # fifteen seconds, no install, touches no data
```
...
> **Public alpha:** `v0.6.8` is available from npm and as an attested GitHub
...
- **Tasq Core** - the embeddable TypeScript library behind the CLI, with no
  opinion about your domain. `@tasq-run/core@0.6.8` exposes the high-level `createLocalTasq`
...
replication, `@tasq-run/client`, the remote CLI and `tasq-remote==0.6.8` are
...
Published `v0.6.8` implements opt-in independent completion resolution across
...
`@tasq-run/console`, and `@tasq-run/client`. Version `0.6.8` is published from
protected GitHub Actions
OIDC with npm provenance; native assets, checksums, SBOMs and attestations are
on the [`v0.6.8` release](https://github.com/gwendall/tasq/releases/tag/v0.6.8).
