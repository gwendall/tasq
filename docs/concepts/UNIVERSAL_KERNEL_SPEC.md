...
[`../contracts/UNIVERSAL_COMPATIBILITY_INVENTORY.json`](../contracts/UNIVERSAL_COMPATIBILITY_INVENTORY.json).
...
`../contracts/TQ-201_EFFECT_AUTHORITY_THREAT_MODEL.md` is the accepted K2 threat model. It
freezes the fail-closed control sequence and adversarial gates.
`../decisions/ADR-002_EFFECT_REQUEST_IDENTITY.md` now accepts canonical request bytes and
...
- effect request canonicalization and the full K2 threat model are accepted in
  `../decisions/ADR-002_EFFECT_REQUEST_IDENTITY.md` and
  `../contracts/TQ-201_EFFECT_AUTHORITY_THREAT_MODEL.md`; effect-capable remote identity and
...
- replica conflicts, tombstones and causal ordering are now accepted in
  `../decisions/ADR-003_REPLICA_CONFLICT_MODEL.md`; TQ-405 implements the neutral commitment
