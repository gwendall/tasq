...
`../contracts/UNIVERSAL_COMPATIBILITY_INVENTORY.json`. UK-003's additive universal registry
...
`../contracts/TQ-401_TRANSACTIONAL_OUTBOX.md`. The CLI consumes this state on startup and
close through the ordered lease protocol in `../contracts/TQ-402_OUTBOX_DRAIN_AND_REPAIR.md`;
...
`../contracts/TQ-403_DURABLE_IDEMPOTENCY.md`.
...
See `../decisions/ADR-003_REPLICA_CONFLICT_MODEL.md` and
`../contracts/TQ-405_EXPLICIT_REPLICATION.md`.
...
`../contracts/TQ-406_SYNC_CHAOS_AND_RECOVERY.md`.
...
tasq-server (private) ──→ tasq-authority ──→ tasq-schema
       └───────────────────────────────→ tasq-core
```
...
See `../contracts/TQ-203_EFFECT_LEDGER_AND_AUTHORITY.md` and
`../contracts/TQ-205_CONNECTOR_DISPATCH_GATE.md` and
`../contracts/TQ-206_EFFECT_RECEIPTS_AND_COMPENSATION.md`.
...
`TASQ_ZERO.md §3` for the original historical decision and `../roadmap/BACKLOG.md` for the
...
`../decisions/ADR-004_AUTHENTICATED_HOSTED_TENANCY.md` is the accepted identity,
