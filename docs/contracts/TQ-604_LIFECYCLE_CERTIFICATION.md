...
**Status:** complete for `v0.3.0`; exact published bytes certified on both
supported targets
...
```bash
bun tasq-v0.3.0-darwin-arm64.install.ts install \
  --archive tasq-v0.3.0-darwin-arm64.tar.gz \
  --manifest tasq-v0.3.0-darwin-arm64.release.json \
  --checksums tasq-v0.3.0-darwin-arm64.SHA256SUMS \
  --prefix "$HOME/.local"

bun tasq-v0.3.0-darwin-arm64.install.ts activate \
  --version 0.3.0 --target darwin-arm64 --prefix "$HOME/.local"

bun tasq-v0.3.0-darwin-arm64.install.ts uninstall \
  --version 0.3.0 --target darwin-arm64 --prefix "$HOME/.local"
```

Version `0.3.0` is published at the immutable GitHub release. Linux uses target
`linux-x64-gnu`.
...
downloaded the exact `v0.3.0` release, verified every GitHub artifact
attestation and ran the complete lifecycle on `darwin-arm64` and
`linux-x64-gnu`. It also replayed the format-5-to-format-26 migration, installed
